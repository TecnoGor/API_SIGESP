...
AS $function$
	BEGIN
		...
		SELECT	
			-- CLIENTE			
			CONCAT_WS('', TRIM(UPPER(COALESCE(NULLIF(f.tipo_documento, ''), ''))), TRIM(UPPER(COALESCE(NULLIF(f.documento, ''), '')))) ::varchar AS rif,
			...
			UPPER(COALESCE(NULLIF(f.direccion, ''), '')) ::varchar AS direccion,
			
			'0000000000'::varchar AS telefono,
			UPPER('mail@mail.com')::varchar AS email,			
			
			-- FACTURA
			f.facturacion_id::int AS id_factura, 
			(f.monto_total - f.iva)::float AS sub_total,
...
-- DROP FUNCTION public.respaldo_fn_api_integracion_get_facturas_por_enviar();

CREATE OR REPLACE FUNCTION public.respaldo_fn_api_integracion_get_facturas_por_enviar()
